...
- YouTube: server reads YouTube metadata and streams MP3, WAV, or MP4 downloads only on localhost.
...
4. Build YouTube.
- Keep this local-server-only because public Cloud Run traffic is not a reliable or appropriate YouTube download source.
- Split the server into request validation, metadata lookup, stream selection, and response streaming.
- Test invalid format, public-host blocking, and URL handling without touching the network.
...
- Server services own YouTube and FFmpeg details behind the localhost-only boundary.
